...
| `commands/caveman.toml`, `commands/caveman-commit.toml`, `commands/caveman-review.toml` | **Vendored at this pin, then dropped.** Claude Code globs `commands/` for `.md` and ignores every other extension without reporting it, so these three registered nothing — measured here, not inferred: they never appeared as slash commands, and `claude plugin validate` passes a plugin containing them. Each also duplicates a skill that does load (`skills/caveman`, `skills/caveman-commit`, `skills/caveman-review`), and the `/caveman…` phrasings keep working through the `UserPromptSubmit` hook regex, which reads the raw prompt and never consulted these files. Settles open question 3 in `README.md`. Other harnesses do read `.toml`, so this is a Claude-Code-specific exclusion: restore them if this plugin is ever packaged for Codex or Gemini. |
